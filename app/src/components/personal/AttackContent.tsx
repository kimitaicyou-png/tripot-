...
const MOCK_EIGHT_CARDS: EightCard[] = [];

const MOCK_TRACKING: TrackingEvent[] = [];

function getTrackingByTarget(targetId: string): TrackingEvent[] {
  return MOCK_TRACKING.filter((e) => e.targetId === targetId).sort(
    (a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()
  );
}

function isWithin48h(timestamp: string): boolean {
...
function scoreCards(cards: EightCard[], tracking: TrackingEvent[]): AttackTarget[] {
      ...
      {events.map((event) => {
        ...
        return (
          <div key={event.id} className="flex items-start gap-1.5">
            <span className={`mt-0.5 shrink-0 ${recent ? 'text-blue-600' : 'text-gray-500'}`}>{cfg.icon}</span>
...
function ImportModal({ onClose, onImport }: { onClose: () => void; onImport: (cards: EightCard[]) => void }) {
  ...
  const [selected, setSelected] = useState<Set<string>>(new Set(MOCK_EIGHT_CARDS.map((c) => c.id)));
  ...
  const handleImport = () => {
    ...
    setTimeout(() => {
      setStep('done');
      setTimeout(() => {
        onImport(MOCK_EIGHT_CARDS.filter((c) => selected.has(c.id)));
        onClose();
      }, 800);
    }, 1500);
  };
        ...
        {step === 'select' && (
          <div className="p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setSelected(selected.size === MOCK_EIGHT_CARDS.length ? new Set() : new Set(MOCK_EIGHT_CARDS.map((c) => c.id)))}
                className="text-xs text-blue-700 font-semibold hover:text-blue-900">
                {selected.size === MOCK_EIGHT_CARDS.length ? '全解除' : '全選択'}
              </button>
            </div>
            <div className="space-y-1.5 max-h-80 overflow-y-auto">
              {MOCK_EIGHT_CARDS.map((card) => (
                <label key={card.id} className={`flex items-start gap-3 p-3 rounded-xl cursor-pointer transition-colors ${selected.has(card.id) ? 'bg-blue-50 border border-blue-200' : 'bg-gray-50 border border-transparent hover:bg-gray-100'}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-600 font-medium">{card.position}</span>
                    </div>
            ...
            <button onClick={handleImport} disabled={selected.size === 0}
              className="w-full mt-4 py-3 bg-yellow-500 text-yellow-900 rounded-xl text-sm font-semibold hover:bg-yellow-400 disabled:bg-gray-300 disabled:text-gray-600 transition-colors">
              {selected.size}件を取り込んでAIスコアリング
            </button>
          </div>
        )}
      ...
      <div className="bg-white rounded-2xl max-w-lg w-full my-8 shadow-sm" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <button className="flex-1 py-2.5 bg-gray-200 text-gray-800 rounded-xl text-sm font-semibold hover:bg-gray-300">コピー</button>
          </div>
        </div>
      </div>
    </div>
...
function loadTargets(): AttackTarget[] {
...
function saveTargets(targets: AttackTarget[]) {
  if (typeof window === 'undefined') return;
...
export function AttackContent() {
  ...
  const handleImport = (cards: EightCard[]) => {
    ...
    setTargets((prev) => {
      ...
      const fresh = newTargets.filter((t) => !existing.has(`${t.name}__${t.company}`));
      const dupeCount = newTargets.length - fresh.length;

      return [...prev, ...fresh];
    });
  };

  const hotTargetIds = new Set(
    MOCK_TRACKING
      .filter((e) => isWithin48h(e.timestamp))
      .map((e) => e.targetId)
  );
  ...
  const promoteToDealIfNeeded = (target: AttackTarget, status: AttackTarget['status']) => {
    ...
    try {
      ...
      if (deals.some((d: { id: string }) => d.id === dealId)) {
        if (status === 'dealt') {
          updateDealForAttack(dealId, { stage: 'proposal' });
        }
        return;
      }
      const newDeal = {
        ...
        dealName: `${target.company} 新規案件`,
        revenueType: 'shot' as const,
        industry: target.industry || 'その他',
        stage: status === 'dealt' ? 'proposal' as const : 'meeting' as const,
        amount: 0,
        probability: status === 'dealt' ? 50 : 30,
        assignee: '柏樹 久美子',
        lastDate: new Date().toISOString().slice(0, 10),
        memo: target.memo || `アタックリストから${status === 'dealt' ? '案件化' : '商談化'}`,
      };
      addDealForAttack(newDeal);
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-5">
      {conversionMsg && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 bg-blue-600 text-white text-sm font-semibold px-4 py-2 rounded-full shadow-sm">
          ✓ {conversionMsg}
        </div>
      )}
      ...
      {targets.length === 0 ? (
        <div className="text-center py-12">
          <div className="w-20 h-20 bg-yellow-100 rounded-2xl mx-auto mb-4 flex items-center justify-center">
            <span className="text-4xl font-semibold text-yellow-600">8</span>
          </div>
          ...
          <button onClick={() => setShowImport(true)}
            className="px-6 py-3 bg-yellow-500 text-yellow-900 rounded-xl text-base font-semibold hover:bg-yellow-400 transition-colors">
            Eightから取り込み
          </button>
        </div>
      ) : (
        <>
              ...
              {(['S', 'A', 'B', 'C'] as const).map((p) => {
                ...
                return (
                  <div key={p} className="text-center bg-white border border-gray-200 rounded-lg p-2">
                    <div className={`w-8 h-8 ${cfg.bg} rounded-lg mx-auto mb-1 flex items-center justify-center`}>
                      <span className={`text-sm font-semibold ${cfg.text}`}>{p}</span>
                    </div>
            ...
            <div className="flex gap-1 bg-gray-100 rounded-lg p-0.5 flex-wrap">
              {(['all', 'S', 'A', 'B', 'C'] as const).map((k) => (
                <button key={k} onClick={() => setFilterPriority(k)}
                  className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${filterPriority === k ? 'bg-white text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}>
                  {k === 'all' ? '全て' : k}
                </button>
              ))}
            </div>
            <button onClick={() => setShowImport(true)}
              className="flex items-center gap-1 px-3 py-1.5 bg-yellow-100 text-yellow-800 rounded-lg text-xs font-semibold border border-yellow-300 hover:bg-yellow-200">
              <span className="font-semibold">8</span> 追加取込
            </button>
          </div>
          ...
          <div className="space-y-2">
            {filtered.map((target) => {
              ...
              const trackingEvents = getTrackingByTarget(target.id);
              return (
                <div key={target.id} className={`bg-white border rounded-lg p-4 ${isHot ? 'border-blue-200' : 'border-gray-200'}`}>
                  <div className="flex items-start gap-3">
                    <div className={`w-9 h-9 ${pc.bg} rounded-lg flex items-center justify-center shrink-0`}>
                      <span className={`text-sm font-semibold ${pc.text}`}>{target.priority}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        {isHot && (
                          <span className="text-xs font-semibold px-1.5 py-0.5 rounded-full bg-blue-100 text-blue-600">アクティブ</span>
                        )}
                      </div>
                      ...
                      {trackingEvents.length > 0 && (
                        <div className="mt-2">
                          <p className="text-xs text-gray-500 font-semibold mb-1">トラッキング活動</p>
                          <TrackingTimeline targetId={target.id} />
                        </div>
                      )}
                      <div className="flex gap-1.5 mt-2">
                        <button onClick={() => setEmailTarget(target)}
                          className="px-2.5 py-1 bg-green-50 text-green-800 rounded-lg text-xs font-semibold border border-green-200 hover:bg-green-100">
                          メール作成
                        </button>
                        <button onClick={() => updateStatus(target.id, 'contacted')}
                          className="px-2.5 py-1 bg-blue-50 text-blue-800 rounded-lg text-xs font-semibold border border-blue-200 hover:bg-blue-100">
                          連絡済み
                        </button>
                        <button onClick={() => updateStatus(target.id, 'meeting')}
                          className="px-2.5 py-1 bg-amber-50 text-amber-800 rounded-lg text-xs font-semibold border border-amber-200 hover:bg-amber-100">
                          商談化
                        </button>
                        <button onClick={() => updateStatus(target.id, 'dealt')}
                          className="px-2.5 py-1 bg-purple-50 text-purple-800 rounded-lg text-xs font-semibold border border-purple-200 hover:bg-purple-100">
                          案件化
                        </button>
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-lg font-semibold text-gray-900">{target.score}</p>
                      <p className="text-xs text-gray-600 font-semibold">スコア</p>
                    </div>
                  </div>
                </div>
              );
