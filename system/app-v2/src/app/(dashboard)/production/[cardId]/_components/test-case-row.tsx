...
export function TestCaseRow(props: Props) {
  ...
  return (
    <li className="flex items-start gap-3 px-4 py-3 bg-card border border-border rounded">
      <div className="flex-1 min-w-0">
        <div className="flex items-baseline gap-3 flex-wrap">
          <p className="text-sm text-ink">{props.title}</p>
          {status === 'pass' && (
            <span className="text-xs text-kpi-up font-medium">✓ PASS</span>
          )}
          {status === 'fail' && (
            <span className="text-xs text-kpi-down font-medium">✗ FAIL</span>
          )}
          {status === null && (
            <span className="text-xs text-subtle">未実行</span>
          )}
        </div>
        {props.expected && (
          <p className="text-xs text-subtle mt-0.5">期待：{props.expected}</p>
        )}
        {props.last_run_at && (
          <p className="text-xs font-mono tabular-nums text-subtle mt-0.5">
