  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="px-6 py-8 max-w-3xl mx-auto space-y-6">
        <section className="bg-white border border-gray-200 rounded-xl p-6 flex items-center gap-5">
        ...
        <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            <p className="text-xs text-gray-500">売上累計</p>
            <p className="font-semibold text-2xl text-gray-900 mt-1 tabular-nums">
              {formatYen(totalRevenue)}
            </p>
          </div>
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            <p className="text-xs text-gray-500">進行中</p>
            <p className="font-semibold text-2xl text-gray-900 mt-1 tabular-nums">{activeDeals} 件</p>
          </div>
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            <p className="text-xs text-gray-500">直近7日 行動量</p>
            <p className="font-semibold text-2xl text-gray-900 mt-1 tabular-nums">{actionStats?.total ?? 0}</p>
          </div>
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            <p className="text-xs text-gray-500">タスク</p>
            <p className="font-semibold text-2xl text-gray-900 mt-1 tabular-nums">
              {taskCounts?.todo ?? 0}
              <span className="text-sm text-gray-700 ml-2">/ {(taskCounts?.todo ?? 0) + (taskCounts?.done ?? 0)}</span>
            </p>
          </div>
        </section>
        ...
        <section className="bg-white border border-gray-200 rounded-xl p-6">
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              ...
              <p className="font-mono tabular-nums text-2xl text-gray-900 mt-1">{actionStats?.calls ?? 0}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500">商談</p>
              <p className="font-mono tabular-nums text-2xl text-gray-900 mt-1">{actionStats?.meetings ?? 0}</p>
            </div>
            <div>
        ...
        <section className="bg-white border border-gray-200 rounded-xl p-6">
          <h3 className="text-sm font-medium text-gray-900 mb-3">
            担当案件 <span className="text-xs text-gray-500 font-normal">{memberDeals.length}件</span>
          </h3>
          {memberDeals.length === 0 ? (
            <p className="text-sm text-gray-700">担当案件はまだありません</p>
          ) : (
