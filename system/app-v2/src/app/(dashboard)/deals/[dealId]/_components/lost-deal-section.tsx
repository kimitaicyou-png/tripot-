...
export async function LostDealSection({
  ...
  if (!isLost && !record) {
    return (
      <section className="bg-white border border-gray-200 border-l-2 border-l-red-200 rounded-xl p-6 space-y-3">
        <div>
    ...
    <section className="bg-red-50 border border-red-200 rounded-xl p-6 space-y-4">
      <div>
        ...
        {record ? (
          <div className="mt-3 space-y-2">
            ...
            {record.competitor && (
              <p className="text-sm text-gray-700">
                <span className="text-xs uppercase tracking-widest text-gray-500 mr-2">競合</span>
                {record.competitor}
              </p>
            )}
            {record.detail && (
              <p className="text-sm text-gray-700 whitespace-pre-wrap">{record.detail}</p>
            )}
          ...
          <p className="text-sm text-gray-700 mt-2">stage=lost ですが詳細未記録</p>
        )}
      </div>
      <LostDealForm
        dealId={dealId}
