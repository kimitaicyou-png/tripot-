  ...
  return (
    ...
    >
      <input
        ...
        className="col-span-7 px-3 py-2 text-sm text-ink bg-card border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-ink/20"
      />
      <input
        type="date"
        name="due_date"
        className="col-span-3 px-3 py-2 text-sm text-ink bg-card border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-ink/20"
      />
      <Button type="submit" variant="primary" size="md" className="col-span-2" disabled={pending}>
        {pending ? '…' : '+ 記録'}
      </Button>
      {state.errors?.text && (
        <p className="col-span-12 text-xs text-kpi-down">{state.errors.text.join(' / ')}</p>
      )}
    </form>
  );
}
